...
@router.get("/test_left_panel", response_model=None)
async def get_test_left_panel_data(
    db: Session = Depends(get_db),
    current_user = Depends(AuthDependencies.get_current_user_optional)
):
    """
    Test endpoint for left panel data with restructured logic:
    - Top 2 reviews: Based on reaction_count + comment_count + view_count
    - Top 2 categories: Categories from user's reviewed entities
    ...
    try:
        ...
        # Debug logging
        import logging
        logging.info(f"[TEST LEFT PANEL] Request received")
        ...
        # 3. Get top reviewers from core_users.review_count
        ...
        return JSONResponse(content={
            ...
            "message": "Test left panel data retrieved successfully"
        })
        
    except Exception as e:
        import logging
        logging.error(f"[TEST LEFT PANEL ERROR] {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch test left panel data: {str(e)}"
        )


@router.get("/reviewinn_left_panel", response_model=None)
async def get_reviewinn_left_panel_data(
    db: Session = Depends(get_db),
    current_user = Depends(AuthDependencies.get_current_user_optional)
):
    """
    ReviewInn-specific endpoint for left panel data with same structure as test_left_panel:
    - Top 2 reviews: Based on reaction_count + comment_count + view_count
    - Top 2 categories: Categories from user's reviewed entities
    - Top reviewers: Based on core_users.review_count
    """
