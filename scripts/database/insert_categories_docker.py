...
def insert_categories():
    ...
    conn = psycopg2.connect(
        ...
        user="review_user",
        password="ReviewInn2024!SecurePass#Dev"
    )
    cur = conn.cursor()
    
    # Load JSON data
    with open('/tmp/category_structure.json', 'r') as f:
        data = json.load(f)
    
    def insert_category_recursive(category_data, parent_id=None, level=1, path_prefix="", sort_order=1):
        ...
        if isinstance(category_data, str):
            ...
            """, (
                ...
                icon, color, True, sort_order, datetime.now(), datetime.now()
            ))
            ...
            if 'category' in category_data:
                for i, subcategory in enumerate(category_data['category']):
                    insert_category_recursive(subcategory, category_id, level + 1, path, i + 1)
    ...
    try:
        ...
        for i, root_category in enumerate(root_categories):
            insert_category_recursive(root_category, None, 1, "", i + 1)
