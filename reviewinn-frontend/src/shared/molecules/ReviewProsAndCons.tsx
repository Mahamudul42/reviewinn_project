...
const ReviewProsAndCons: React.FC<ReviewProsAndConsProps> = ({
  ...
  return (
    <div className={`grid grid-cols-2 gap-2 min-w-0 ${className}`}>
      {/* Pros Section */}
      {displayedPros.length > 0 && (
        <div className="bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-200 rounded-xl p-3 shadow-sm min-w-0 overflow-hidden">
          <ul className="space-y-2.5">
            {displayedPros.map((pro, index) => (
              <li key={index} className="text-emerald-800 text-sm flex items-start gap-3 min-w-0">
                <div className="w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                  <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                  </svg>
                </div>
                <span className="flex-1 font-medium leading-relaxed break-words min-w-0">{pro}</span>
              </li>
            ))}
          </ul>
      ...
      {displayedCons.length > 0 && (
        <div className="bg-gradient-to-br from-rose-50 to-red-50 border border-rose-200 rounded-xl p-3 shadow-sm min-w-0 overflow-hidden">
          <ul className="space-y-2.5">
            {displayedCons.map((con, index) => (
              <li key={index} className="text-rose-800 text-sm flex items-start gap-3 min-w-0">
                ...
                <span className="flex-1 font-medium leading-relaxed break-words min-w-0">{con}</span>
              </li>
            ))}
          </ul>
