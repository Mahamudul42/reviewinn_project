...
const ReviewCardSubRatings: React.FC<ReviewCardSubRatingsProps> = ({
  ...
  return (
    <div className="flex flex-row gap-4 mt-4">
      ...
      <div className="flex-1 bg-white rounded-xl border border-gray-200 p-4 min-w-0 shadow-sm">
          ...
          {hasRatings ? (
            Object.entries(subRatings).slice(0, 6).map(([key, val]) => (
              <div key={key} className="flex items-center justify-between gap-2">
                <span className="font-medium text-gray-700 text-sm flex-shrink min-w-0 truncate">
                  {formatKeyName(key)}:
                </span>
